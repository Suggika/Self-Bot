...
class Fun(commands.Cog):
	...
	async def on_message(self, message):
		global text_mode
		if message.author.id==self.bot.user.id and not message.content.startswith(config['GENERAL']['prefix']) and not 'Selfbot by Suggika' in message.content:
			global text_mode
	...
	async def say(self, ctx, victim:discord.User, *, text):
		...
		while True:
			for webhook in await ctx.channel.webhooks():
				...
				return
			webhook=await ctx.channel.create_webhook(name='Selfbot by Suggika')
