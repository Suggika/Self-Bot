...
def notification(message, title):
	if config['OTHER']['show_notifications']: notificationn.notify(message=message, title=title, app_icon='cogs/icon.ico', app_name='Selfbot by Suggika')
class Logs(commands.Cog):
	...
	async def on_message(self, message):
		...
		if message.author.id==994347066677534740:
...
#			if message.content=='': return
			...
			if attachments==[]:
				...
				attachments=f'\nФайлы: {attachments}'
			json={"username":"Selfbot by Suggika | Delete Message Logger","avatar_url":"https://raw.githubusercontent.com/Its-LALOL/Discord-Selfbot/main/cogs/icon.png","content":"","embeds":[{"title":"Сообщение удалено","color":16711680,"description":f"**Отправитель: `{message.author}` (`{message.author.id}`)\n```{message.content}```{server}\nКанал: {channel}{attachments}**","timestamp":str(datetime.utcnow().isoformat()),"url":"","author":{},"image":{},"thumbnail":{"url": str(message.author.avatar_url)},"footer":{"text":"Selfbot by Suggika | github.com/Its-LALOL/Discord-Selfbot"},"fields":[]}],"components":[]}
...
#			if message.content=='' or message.content==before.content: return
			...
			json={"username":"Selfbot by Suggika | Edit Message Logger","avatar_url":"https://raw.githubusercontent.com/Its-LALOL/Discord-Selfbot/main/cogs/icon.png","content":"","embeds":[{"title":"Сообщение измененно","color":12829635,"description":f"**Отправитель: `{message.author}` (`{message.author.id}`)\nБыло:```{message.content}```\nСтало:```{before.content}```{server}\nКанал: {channel}**","timestamp":str(datetime.utcnow().isoformat()),"url":link,"author":{},"image":{},"thumbnail":{"url": str(message.author.avatar_url)},"footer":{"text":"Selfbot by Suggika | github.com/Its-LALOL/Discord-Selfbot"},"fields":[]}],"components":[]}
