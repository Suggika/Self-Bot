...
class Tools(commands.Cog):
	...
	async def status(self, ctx, cat='ы', *, name='Selfbot by Suggika'):
		...
		if cat in game:
			...
			await self.bot.change_presence(status=sstatus, activity=discord.Streaming(name=name, url="https://www.youtube.com/watch?v=itFnr-M-MAU"))
		elif cat in reset:
			...
			await self.bot.change_presence(status=sstatus, activity=discord.Activity(type=discord.ActivityType.playing, application_id=1032671485120229397, name='Selfbot by Suggika', details='bit.ly/3CXNRpC', assets={'large_image': '1032672678106116216', 'large_text': 'Selfbot by LALOL\nbit.ly/3CXNRpC'}))
		else:
			await ctx.message.edit(content="**__Selfbot by Suggika__\n\nДоступные варианты: ```Обычные````Watching`, `Listening`, `Playing`, `Streaming` и `Reset`\n\n```Эксклюзивные````Discord`, `Selfbot`**")
			return
	...
	async def purge(self, ctx, amount: int):
		...
		for message in messages:
			if message.author.id==self.bot.user.id:
				...
				deleted+=1
		await ctx.send(f"**__Selfbot by Suggika__\n\n:broom: Успешно удалил {deleted} сообщений!**")
		...
		for i in range(amount):
			while True:
				...
				if response.status_code==200 or response.status_code==201:
					id=response.json()['id']
					requests.post(f"https://discord.com/api/v9/channels/{id}/messages", headers={'authorization': self.bot.http.token}, json={"content": "||@everyone|| **__Selfbot by Suggika\nhttps://github.com/Its-LALOL/Discord-Selfbot __**"})
					break
				elif response.status_code==429:
					seconds=response.json()['retry_after']
					if 100>seconds:
					...
					await ctx.send(content=f"**__Selfbot by Suggika__\n\nПроизошла ошибка :x:\n```Код ошибки: {response.status_code}\n{response.text}```**")
					return
	...
	async def copystatus(self, ctx, user:discord.Member):
		...
		await ctx.message.edit(content=f"**__Selfbot by Suggika__\n\n:jigsaw: Успешно скопировал статус у `{user}`!**")
	...
	async def translate(self, ctx, to='ru', *, text=None):
		...
		if dest=='en': dest='gb'
		await ctx.message.edit(content=f'**__Selfbot by SuggikaL__\n\n:flag_{src}:: `{text}`\n:flag_{dest}:: `{translation.text}`**')
	@commands.command()
