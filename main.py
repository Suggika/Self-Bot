#Селф-бот BETA by Suggika. Баги, ошибки? Пишите в дс @suggika

version=1.0
lencommands=0
import os
clear=lambda: os.system(f'cls && title Selfbot by Suggika {version} - {lencommands} Commands' if os.name == 'nt' else 'clear')
try:
	import discord
...
if theme=='standart':
	color={'Intro': Fore.MAGENTA, 'Info_name': Fore.YELLOW, 'Info_value': Fore.WHITE}
elif theme=='discord':
...
try: bot=commands.Bot(command_prefix=pref, case_insensitive=True, self_bot=True)
	...
	sleep(3)
	webopen('https://github.com/Suggika/SelfBot-by-Suggika-v-1.0/tree/main', 2)
	while True: sleep(9)
...
#	status=config['GENERAL']['status']
	...
	if float(requests.get('https://github.com/Suggika/SelfBot-by-Suggika-v-1.0/blob/main/cogs/version').text)>version:
		global update
		update=f':warning: Пожалуйста, обновите селфбота используя команду {pref}stopall**\n**'
		print(f'{Fore.RED}Пожалуйста, обновите селфбота используя команду {Fore.WHITE}{pref}stopall{Fore.RESET}{Fore.RED}\n')
		return
	print(Fore.RED)
if on_command_error:
	@bot.event
	async def on_command_error(ctx, error):
		...
		try: await ctx.send(f'**__Selfbot by Suggika__\n\nПроизошла ошибка :x:\n```{error}```**')
		except: pass
@bot.event
...
async def __bot(ctx):
	await ctx.message.edit(content='**__Selfbot by Suggika__\n\nСсылка: https://github.com/Suggika/SelfBot-by-Suggika-v-1.0/tree/main **')
